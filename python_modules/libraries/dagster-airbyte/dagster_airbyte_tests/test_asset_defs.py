...
def test_assets(schema_prefix):
    ...
    assert materializations[0].metadata["bytesEmitted"] == 1234
    assert materializations[0].metadata["recordsCommitted"] == 4321
    assert materializations[0].metadata["schema"] == TableSchema(
        columns=[
